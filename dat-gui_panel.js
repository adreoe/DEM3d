Q3D.gui = {
  ...
  addCustomPlaneFolder: function () {
    ...
    // Plane color
    folder.addColor(parameters.cp, 'c').name('Цвет').onChange(function (value) {
    ...
    // Plane opacity
    folder.add(parameters.cp, 'o').min(0).max(1).name('Прозрачность (0-1)').onChange(function (value) {
      if (customPlane === undefined) addPlane(parameters.cp.c);
      customPlane.material.opacity = value;
    });

    // Enlarge plane option
    folder.add(parameters.cp, 'l').name('Увеличить').onChange(function (value) {
      if (customPlane === undefined) addPlane(parameters.cp.c);
  ...
  addCommandsFolder: function () {
    var folder = this.gui.addFolder('Команды');
    if (Q3D.Controls.type == "OrbitControls") {
      folder.add(this.parameters.cmd, 'rot').name('Авто-вращение').onChange(function (value) {
  ...
  addHelpButton: function () {
    this.gui.add(this.parameters, 'i').name('Помощь');
  }
};
